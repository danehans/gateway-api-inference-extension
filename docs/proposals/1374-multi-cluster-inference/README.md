...
InferencePool selection is implementation-specific. The following are examples of how an implementation may select routing to an exported InferencePool:
...
**Note:** When an exported InferencePool is selected by a IG, standard EPP semantics are used to select endpoints of that pool.
...
the example InferencePoolImport and the implementation would be responsible for balancing the traffic.
