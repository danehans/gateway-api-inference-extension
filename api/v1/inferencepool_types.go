...
type InferencePoolSpec struct {
	// Selector determines which Pods are members of the InferencePool. It matches
	// Pods by their labels only within the same namespace; cross-namespace selection
	// is not supported.
	//
	// The structure of this LabelSelector is intentionally simple to be compatible
	// with Kubernetes Service selectors, as some implementations may translate this
	// configuration into a Service resource.
	//
	// +required
	Selector LabelSelector `json:"selector"`

	// TargetPorts defines a list of ports that are exposed by the InferencePool.
	...
	// The endpoint picker is responsible for selecting the appropriate model server
	// Pods.
	//
	// +required
...
// Port defines the network port that will be exposed by the InferencePool.
type Port struct {
...
// ExtensionFailureMode defines the options for how the gateway handles the case when the
// extension is not responsive.
//
// +kubebuilder:validation:Enum=FailOpen;FailClose
type ExtensionFailureMode string

const (
	// FailOpen specifies that the proxy should forward the request to an endpoint of its picking
	// when the Endpoint Picker fails.
...
type InferencePoolStatus struct {
	// Parents is a list of parent resources (usually Gateways) that are associated with
	// the InferencePool, and the status of the InferencePool with respect to each parent.
	//
	// A controller that manages the InferencePool, must add an entry for each parent when
	// setting the "Accepted" condition. A parent is typically represented as a Gateway.
	//
	// A maximum of 32 parents will be represented in this list. When the list is empty,
	// it indicates that the InferencePool is not associated with any parents. An implementation
	// MUST add and remove parent entries.
	//
	// +kubebuilder:validation:MaxItems=32
	// +optional
	// +listType=atomic
	Parents []PoolStatus `json:"parents,omitempty"`
}
...
// ParentReference identifies an API object including its namespace. It is used to match
// the InferencePool with a parent resource, such as a Gateway.
type ParentReference struct {
	// Group is the group of the referent. When unspecified, the referent is assumed to be
	...
	// Kind is kind of the referent. When unspecified, the referent is assumed to be a
	// "Gateway" kind.
	//
	// +optional
